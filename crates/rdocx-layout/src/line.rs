...
pub struct TextSegment {
    ...
    /// Byte offset in `text` that each glyph came from, straight from the
    /// shaper. Required to slice this segment correctly, because shaping is
...
fn split_text_subsegment(seg: &TextSegment, byte_start: usize, byte_end: usize) -> InlineItem {
    ...
    // Select glyphs by the shaper's own cluster values, which say the byte
    // each glyph came from. Slicing by character index instead only works
    // when shaping happens to be one glyph per character, and estimating a
    // proportional range when it is not made neighbouring chunks overlap, so
    // a glyph was drawn twice and a letter appeared doubled.
    let (sub_glyphs, sub_advances, sub_width) =
        if seg.clusters.len() == seg.glyph_ids.len() && !seg.clusters.is_empty() {
            let mut glyphs = Vec::new();
            let mut advances = Vec::new();
            for (i, &cluster) in seg.clusters.iter().enumerate() {
                let c = cluster as usize;
                if c >= byte_start && c < byte_end {
                    glyphs.push(seg.glyph_ids[i]);
                    advances.push(seg.advances[i]);
                }
            }
            ...
            // No cluster data, which should not happen for anything the shaper
            // produced. Fall back to a 1:1 slice rather than guessing a range.
            ...
            let glyphs = seg.glyph_ids[start..end].to_vec();
            let advances = seg.advances[start..end].to_vec();
            let width: f64 = advances.iter().sum();
            (glyphs, advances, width)
        };

    // A cluster is attributed to the chunk containing its starting byte, so
    // a ligature spanning a break lands wholly in the first chunk. Its glyph
    // is drawn once, which is the point.
    let sub_clusters: Vec<u32> = seg
        .clusters
        .iter()
        .copied()
        .filter(|&c| (c as usize) >= byte_start && (c as usize) < byte_end)
        .map(|c| c - byte_start as u32)
        .collect();
...
fn shape_leader(
    ...
    Some(TextSegment {
        ...
        glyph_ids,
        advances,
        // Leader glyphs are generated by repetition, not shaped from the
        // text, so there is no cluster mapping to carry.
        clusters: Vec::new(),
...
mod tests {
    ...
    /// Build a segment whose shaping is not one glyph per character, the way
    /// a font with an "fi" ligature shapes "financial": 9 characters, 8
    /// glyphs, and the first glyph covering two characters.
    fn ligature_segment() -> TextSegment {
        let text = "financial";
        // cluster = byte the glyph came from. The fi ligature covers bytes 0
        // and 1, so no glyph reports cluster 1.
        let clusters = vec![0, 2, 3, 4, 5, 6, 7, 8];
        let mut seg = make_text_segment(text, 90.0);
        seg.glyph_ids = (0..clusters.len() as u16).map(|i| i + 100).collect();
        seg.advances = vec![10.0; clusters.len()];
        seg.clusters = clusters;
        seg
    }

    /// Splitting a shaped segment must never emit the same glyph twice.
    ///
    /// Glyphs used to be selected by slicing on character index, with a
    /// proportional guess when shaping was not one glyph per character.
    /// Neighbouring chunks then overlapped and a glyph was drawn in both, so
    /// a letter appeared doubled on the page.
    #[test]
    fn splitting_a_shaped_segment_never_repeats_a_glyph() {
        let seg = ligature_segment();
        let total = seg.glyph_ids.len();

        // Split at every byte boundary and collect what each half emits.
        for cut in 1..seg.text.len() {
            if !seg.text.is_char_boundary(cut) {
                continue;
            }
            let mut seen = Vec::new();
            for (start, end) in [(0, cut), (cut, seg.text.len())] {
                if let InlineItem::Text(part) = split_text_subsegment(&seg, start, end) {
                    seen.extend(part.glyph_ids);
                }
            }
            ...
            assert_eq!(
                sorted.len(),
                seen.len(),
                "cut at {cut} drew a glyph twice: {seen:?}"
            );
            assert_eq!(
                seen.len(),
                total,
                "cut at {cut} lost or gained a glyph: {seen:?}"
            );
        }
    }

    /// A ligature spanning a break belongs to one side of it, not both.
    #[test]
    fn a_ligature_is_not_split_across_chunks() {
        let seg = ligature_segment();
        // Cut between the two characters the ligature covers.
        let InlineItem::Text(first) = split_text_subsegment(&seg, 0, 1) else {
            panic!("expected text");
        };
        let InlineItem::Text(second) = split_text_subsegment(&seg, 1, seg.text.len()) else {
            panic!("expected text");
        };
        assert_eq!(
            first.glyph_ids,
            vec![100],
            "the ligature glyph belongs to the chunk holding its first byte"
        );
        assert!(
            !second.glyph_ids.contains(&100),
            "and must not be repeated in the next chunk"
        );
    }

    /// Clusters are rebased onto the sub-segment, so a chunk can be split
    /// again without the offsets pointing outside its own text.
    #[test]
    fn sub_segment_clusters_are_relative_to_the_sub_segment() {
        let seg = ligature_segment();
        let InlineItem::Text(part) = split_text_subsegment(&seg, 3, 7) else {
            panic!("expected text");
        };
        assert!(
            part.clusters
                .iter()
                .all(|&c| (c as usize) < part.text.len()),
            "clusters {:?} must index into {:?}",
            part.clusters,
            part.text
        );
    }
